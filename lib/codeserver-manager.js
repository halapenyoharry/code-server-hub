...
class CodeServerManager {
  ...
  async startInstance(port) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Add HTTPS certificates if available
      if (hasSSL) {
        args.push('--cert', path.join(SHARED_CERTS_DIR, 'code-server.pem'));
        args.push('--cert-key', path.join(SHARED_CERTS_DIR, 'code-server.key'));
      }
