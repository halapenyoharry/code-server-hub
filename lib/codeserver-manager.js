...
let pathConfig = {};
const PATHS_CONFIG_FILE = path.join(__dirname, '..', 'config', 'paths.json');
try {
  if (fs.existsSync(PATHS_CONFIG_FILE)) {
    pathConfig = JSON.parse(fs.readFileSync(PATHS_CONFIG_FILE, 'utf8'));
  }
} catch (error) {
  console.log('No paths configuration found, using defaults');
}

// Paths for code-server data and instances
const CODE_SERVER_DATA_ROOT = process.env.CODE_SERVER_DATA_DIR || pathConfig.CODE_SERVER_DATA || path.join(__dirname, '..', 'data');
...
class CodeServerManager {
    ...
    try {
      ...
      for (const [port, config] of Object.entries(parsed.instances)) {
        ...
        const resolvedConfig = {
          ...config,
          workspace: this.resolvePath(config.workspace),
        ...
        this.instances.set(port, resolvedConfig);
      }
    } catch (error) {
  ...
  async startInstance(port) {
    ...
    const instanceDir = path.join(INSTANCES_DIR, port.toString());
    ...
    const workspaceDir = instance.workspace || process.env.HOME + '/servers';
    ...
    if (!fs.existsSync(workspaceDir)) {
      ...
      const userCertsDir = process.env.HOME + '/servers/code-server/certificates';
    ...
    // Apply default VS Code settings for better clipboard support
    await this.applyDefaultSettings(instanceDir);

    // Check if certificates exist for HTTPS
    const hasSSL = fs.existsSync(path.join(SHARED_CERTS_DIR, 'code-server.pem')) && 
                   fs.existsSync(path.join(SHARED_CERTS_DIR, 'code-server.key'));

    return new Promise((resolve, reject) => {
      // Use command line args with HTTPS if certificates are available
      // Create a temporary config to override default ~/.config/code-server/config.yaml
      const tempConfigPath = path.join(instanceDir, 'config.yaml');
      const configContent = `bind-addr: 0.0.0.0:${port}
auth: none
cert: ${hasSSL ? path.join(SHARED_CERTS_DIR, 'code-server.pem') : 'false'}
cert-key: ${hasSSL ? path.join(SHARED_CERTS_DIR, 'code-server.key') : ''}
`;
      ...
      const codeServerBin = process.env.CODE_SERVER_BIN || '/opt/homebrew/bin/code-server';
      console.log(`Starting code-server: ${codeServerBin} ${args.join(' ')}`);
      
      // Clean environment to avoid conflicts - especially PORT which is set by the hub
      const cleanEnv = { ...process.env };
      delete cleanEnv.PORT; // Remove PORT to avoid conflicts

      const childProcess = spawn(codeServerBin, args, {
        detached: false,
      ...
      // Store the process
      this.processes.set(port.toString(), childProcess);
      ...
      // Give it more time to start (especially with extensions loading)
      setTimeout(async () => {
        ...
        if (newStatus.running) {
          ...
          let errorDetails = '';
          try {
            errorDetails = fs.readFileSync(errorFile, 'utf8').slice(-500);
          } catch (e) {
            // Ignore read errors
          }
          reject(new Error(`Failed to start instance ${port}. ${errorDetails}`));
        }
      }, 6000);
    });
  }
  ...
  async applyDefaultSettings(dataDir) {
    try {
      ...
      if (!fs.existsSync(defaultSettingsPath)) {
        console.log('Default settings file not found, skipping...');
        return;
      }
      ...
      if (fs.existsSync(settingsPath)) {
        try {
          ...
          console.log('Error reading existing settings, will create new file');
        }
      }
      ...
      console.log('Applied default VS Code settings for better clipboard support');
      
    } catch (error) {
      console.log('Error applying default settings:', error.message);
    }
  }
}
