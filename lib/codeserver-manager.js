...
const CODE_SERVER_DATA_ROOT = '/Users/harold/servers/code-server-data';
...
class CodeServerManager {
  ...
  async startGoTTYInstance(port, instance) {
    try {
      ...
      try {
        await execPromise('which gotty');
      } catch (error) {
        throw new Error('GoTTY is not installed. Install it with: brew install gotty');
      }

      // Build GoTTY command
      const command = instance.command || 'bash';
      const gottyCmd = `gotty -p ${port} -w --reconnect ${command}`;
      
      console.log(`Starting GoTTY on port ${port} with command: ${command}`);
      
      // Start GoTTY in background
      const { stdout } = await execPromise(`nohup ${gottyCmd} > /tmp/gotty-${port}.log 2>&1 & echo $!`);
      ...
      return {
        success: true,
        message: `GoTTY instance started on port ${port}`,
        pid
      };
    } catch (error) {
      throw new Error(`Failed to start GoTTY instance: ${error.message}`);
    }
  }
